...
subtitle: "30-year population change at 57 sites with at least one count (accuracy < 5) prior to 2005 and at least one count (accuracy < 5) after 2004"
...
#library(knitr)
#knit('./supplement_markdown/SuppCode6_MAPPPDreanalysis_inputdatasubset_57sites.Rmd')
```
...
# Subset to sites with counts within 15 years of end points
...
lat_plot = ggplot(data = re, aes(x = Lat, y = estZss))+
  ...
  geom_errorbar(aes(ymin=estZss-seZss,
                    ymax=estZss+seZss))+
  theme_bw()+
  ylab("Slope (rate of population change)")+
...
hist_growth = 
  ggplot(data = data.frame(pred_re$pop_change), aes(x = pred_re.pop_change,
                                                     after_stat(density)))+
  geom_histogram(bins = 40, colour = "black", fill = "grey51")+
  theme_bw()+
...
pop_predict.p = pop_predict.p + 
  ...
  geom_jitter(data = pred_last, aes(x = year2, y = pred_last), 
             col = "black",size = 1, height = 0, width = 0.5, 
              alpha = 0.05, stroke = NA) +
   scale_y_continuous(lim = c(0, 300000), labels = scales::comma) + 
   theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank()) 
...
## Save Plot 
pdf("./figure/MS_Population_change2.pdf",
