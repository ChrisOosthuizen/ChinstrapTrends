...
pop_predict_mc2p = ggplot() + 
  ...
  geom_line(data = pop_predict_mc2, aes(x = year, y = total_pred, 
               color = "Model predicted count"), color =  "darkblue", lty = 1, size = 1)+
    geom_line(data = pop_predict_mc2, aes(x = year, y = min_pred, 
               color = "Model predicted count"), color =  "steelblue", lty = 2, size = 0.8)+
...
fit.mc2p + 
  gg_theme()+
  labs(title = "", subtitle = paste0("MCMCglmm: predict with RE (this study)"))+
...
pop_predict_mc2p  +
  gg_theme()+
  labs(subtitle = "MCMCglmm: predict with RE (this study)")+
...
fit.mc_Kr_randomeff.p = fit.mc_Kr_randomeff.p + 
                      gg_theme()+
                      labs(title = "", 
            subtitle = paste0("Kr",ds4psy::Umlaut["u"], "ger (2023) MCMCglmm: predict with RE"))+
      theme(legend.position="none") + 
    theme(plot.margin = margin(1,6,1,1))
   
pop_predict_Krp_re = pop_predict_Krp_re +
                gg_theme()+
                labs(title = "", 
                subtitle = paste0("Kr",ds4psy::Umlaut["u"], "ger (2023) MCMCglmm: predict with RE")) + 
            theme(legend.position = c(0.7, 0.9)) + 
...
#      useDingbats = FALSE, width = 13, height = 9)
# (fit.mc_Kr_marginp | fit.mc_Kr_randomeff.p | fit.mc2p) /
# (pop_predict_Krp | pop_predict_Krp_re | pop_predict_mc2p) +
#   plot_annotation(tag_levels = 'A') &
#   theme(plot.tag.position = c(0.1, 0.9),  # Adjust position (x, y)
