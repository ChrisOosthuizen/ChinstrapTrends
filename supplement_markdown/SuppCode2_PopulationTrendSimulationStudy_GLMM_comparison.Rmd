...
subtitle: "Analysis of simulated population counts with GLMMs"
...
In Supplement 3, we repeat this analysis with sparse observation data.

```{r run, include=FALSE}
#library(knitr)
#knit('./supplement_markdown/SuppCode2_PopulationTrendSimulationStudy_GLMM_comparison.Rmd')
```
...
# Without random effects, response scale prediction 
...
## MCMCglmm predictions with our model specification (mc2)
...
fit.mc2p = ggplot(df, aes(x = year, y = fit.mc2, color = as.factor(latitude))) +
  ...
  # add observed data
  geom_point(aes(x = year, y = count, color = as.factor(latitude)), size = 0.9)+
  labs(title = "MCMCglmm: predict with RE (this study)",
...
ggplot(df, aes(x = year, y = fit.mc2_margin, color = as.factor(latitude))) +
  ...
  labs(title = "MCMCglmm: prediction with no random effects",
       subtitle = "Relatively poor prediction to sites even with correct\nmodel specification")

# while the fit is relatively poor, latitude explain much of the site level variation in this 
# simulated data set.
...
pop_predict_mc2p = ggplot() + 
  geom_point(data = population, aes(x = year, y = totalN, 
                                    color = "Simulated data"), size = 2.3) +
  geom_line(data = pop_predict_mc2, aes(x = year, y = total_pred, 
                                     color = "Model prediction"), lty = 1, size = 1) +
  
  geom_line(data = pop_predict_mc2, aes(x = year, y = min_pred, 
                                        color = "Model prediction interval"), lty = 2, size = 0.8) +
  ...
  labs(subtitle = "Total observed and predicted counts across 26 sites") +
  guides(color = guide_legend(title = "")) +
  scale_color_manual(values = c("Simulated data" = "red", 
                                "Model prediction" = "darkblue",
                                "Model prediction interval" = "steelblue"),
                     labels = c("Simulated data" = expression(Sigma~"simulated data"),
                                "Model prediction" = expression(Sigma~"posterior mean predictions"),
                                "Model prediction interval" = expression(Sigma~"95% prediction intervals"))) +
  theme(legend.position = c(0.7, 0.8))
...
# Now predict with maginalised random effects and a prediction interval: 
...
pop_predict_Krp = ggplot() + 
  ...
  geom_line(data = pop_predict_Kr, aes(x = year, y = min_pred, 
                                       color = "Model prediction interval (95%)"), lty = 2, size = 0.8) +
  ...
  scale_color_manual(values = c("Simulated data" = "red", 
                                "Model prediction" = "darkblue", 
                                "Model prediction interval (95%)" = "steelblue"),
                     labels = c("Simulated data" = expression(Sigma~"simulated data"),
                                "Model prediction" = expression(Sigma~"posterior mean predictions"),
                                "Model prediction interval (95%)" = expression(Sigma~"95% prediction intervals"))) +
  theme(legend.position = c(0.7, 0.8))
...
# Finally predict with random effects (should yield a better prediction) and a prediction interval: 

pred_Kr_randomeff <- data.frame(predict(mc_Kr, 
                            newdata=df,
                            type="response",
                            marginal=NULL,
                            interval="prediction",
...
df$fit.mc_Kr_randomeff <- pred_Kr_randomeff$fit
df$lwr.mc_Kr_randomeff <- pred_Kr_randomeff$lwr
df$upr.mc_Kr_randomeff <- pred_Kr_randomeff$upr

fit.mc_Kr_randomeff.p = 
ggplot(df, aes(x = year, y = fit.mc_Kr_randomeff, color = as.factor(latitude))) +
  geom_line() +
  labs(x = "Year", y = "Count") +
  scale_color_discrete(name = "Latitude") + 
...
# calculate total population size predicted (total N across all sites)
pop_predict_Kr_re = df %>%
             group_by(year) %>% 
             summarise(total_pred_re = sum(fit.mc_Kr_randomeff),
                       min_pred_re = sum(lwr.mc_Kr_randomeff),
                       max_pred_re = sum(upr.mc_Kr_randomeff))

# Plot total N observed per year and the predicted total abundance per year
pop_predict_Krp_re = ggplot() + 
  geom_point(data = population, aes(x = year, y = totalN, 
                                    color = "Simulated data"), size = 2.3) +
  geom_line(data = pop_predict_Kr_re, aes(x = year, y = total_pred_re, 
                                          color = "Model prediction"), lty = 1, size = 1) +
  geom_line(data = pop_predict_Kr_re, aes(x = year, y = min_pred_re, 
                                          color = "Model prediction interval (95%)"), lty = 2, size = 0.8) +
  geom_line(data = pop_predict_Kr_re, aes(x = year, y = max_pred_re, 
                                          color = "Model prediction interval (95%)"), lty = 2, size = 0.8) +
  labs(x = "Year", y = "Total population count") +
  theme_bw() +
  scale_y_continuous(label = scales::comma) +
...
fit.mc2p + 
  gg_theme()+
  labs(title = "", subtitle = paste0("Current (revised) analysis"))+
...
fit.mc_Kr_marginp + 
  ...
  labs(title = "", subtitle = paste0("Kr",
       ds4psy::Umlaut["u"], "ger (2023)"))+
  theme(legend.position="none") + 
    theme(plot.margin = margin(1,6,1,1))+ 
   theme(panel.grid.major = element_blank(),
    panel.grid.minor = element_blank()) 
...
pop_predict_mc2p2 = pop_predict_mc2p  +
  gg_theme()+
...
pop_predict_Krp2 = pop_predict_Krp  + 
  gg_theme()+
  labs(subtitle = paste0("Kr",ds4psy::Umlaut["u"], "ger (2023)"))+
  theme(legend.position = c(0.6, 0.9)) + 
...
# update: 
fit.mc_Kr_randomeff.p = fit.mc_Kr_randomeff.p + 
                      gg_theme()+
                      labs(title = "", 
            subtitle = paste0("Kr",ds4psy::Umlaut["u"], "ger (2023): predict with RE"))+
      theme(legend.position="none") + 
    theme(plot.margin = margin(1,6,1,1))+ 
   theme(panel.grid.major = element_blank(),
    panel.grid.minor = element_blank())
   
pop_predict_Krp_re2 = pop_predict_Krp_re +
                gg_theme()+
                labs(title = "", 
                subtitle = paste0("Kr",ds4psy::Umlaut["u"], "ger (2023): predict with RE")) + 
            theme(legend.position = c(0.6, 0.9)) + 
...
(fit.mc_Kr_marginp | fit.mc_Kr_randomeff.p | fit.mc2p) / 
(pop_predict_Krp2 | pop_predict_Krp_re2 | pop_predict_mc2p2) + 
  plot_annotation(tag_levels = 'A') & 
  theme(plot.tag.position = c(0.1, 0.9),  # Adjust position (x, y)
        plot.tag = element_text(size = 12, hjust = 0, vjust = 0))

# # Save Plot 
#  pdf("./figure/Figure2.pdf",
...
# 
# # Save as PNG
# ggsave("./figure/Figure2.png",
#        (fit.mc_Kr_marginp | fit.mc_Kr_randomeff.p | fit.mc2p) /
# (pop_predict_Krp | pop_predict_Krp_re | pop_predict_mc2p) +
#   plot_annotation(tag_levels = 'A') &
#   theme(plot.tag.position = c(0.1, 0.9),  # Adjust position (x, y)
#         plot.tag = element_text(size = 12, hjust = 0, vjust = 0)),
# width = 13, height = 9, dpi = 2000)


```

# Supplement 5 figures

In Figure S5.2 we show why we did not use posterior predictive simulation (interval=“prediction”) to compare the 'intecept' model (Figure S5.1.). When prediction intervals and marginalized random effects are specified (this was the syntax implemented by Krüger (2023)) the predictions are highly variable (‘wriggly’) (see Figure S5.2). Also, note that the predictions differ each time we predict using the same GLMM as object (Figure S5.2). Most (but not all) of this variation is removed when we specify prediction intervals but include random effects in the prediction (Figure S5.3). 

```{r suppl 5 figures, include=TRUE, message=FALSE, warning=TRUE, fig.width=11,fig.height=7}
...
aa = ggplot(df, aes(x = year, y = fit.mc_Kr_margin, color = as.factor(latitude))) +
  geom_line() +
  labs(x = "Year", y = "Count") +
  scale_color_discrete(name = "Latitude") + 
  theme_bw() + 
...
bb = ggplot(df, aes(x = year, y = fit.mc_Kr_margin, color = as.factor(latitude))) +
  geom_line() +
  labs(x = "Year", y = "Count") +
  scale_color_discrete(name = "Latitude") + 
  theme_bw() + 
...
cc =  ggplot(df, aes(x = year, y = fit.mc_Kr_margin, color = as.factor(latitude))) +
  geom_line() +
  labs(x = "Year", y = "Count") +
  scale_color_discrete(name = "Latitude") + 
  theme_bw() + 
...
dd = ggplot(df, aes(x = year, y = fit.mc_Kr_margin, color = as.factor(latitude))) +
  geom_line() +
  labs(x = "Year", y = "Count") +
  scale_color_discrete(name = "Latitude") + 
  theme_bw() + 
...
dd = dd + theme(legend.position="none")

cowplot::plot_grid(aa , bb , cc , dd,
                    labels = c('A', 'B', 'C', 'D'), ncol = 2, label_size = 12)

# ## Save Plot 
# pdf("./figure/Supp_Kruger_prediction intervals_without random effects.pdf",
...
# ## Save Plot 
# pdf("./figure/Supp_Kruger_prediction intervals_with random effects.pdf",
#     useDingbats = FALSE, width = 10, height = 10)
# cowplot::plot_grid(aa , bb , cc , dd,
#                    labels = c('A', 'B', 'C', 'D'), ncol = 2, label_size = 12)
# dev.off()
