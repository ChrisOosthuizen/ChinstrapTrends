...
#library(knitr)
#knit('./supplement_markdown/SuppCode1_Kruger2023review_GLMMcomparison_Kruger data.Rmd')
```
...
> The prediction above contains a high degree of uncertainty, which was ignored in the original paper. The uncertainty is the lwr and upr columns, which is the Highest Posterior Density intervals, I believe, from coda::HPDinterval. https://rdrr.io/cran/MCMCglmm/src/R/predict.MCMCglmm.R*   
...
> This figure plots all the individual site level predictions. It cannot be sensible given the poor model fit and predictions shown above. On some model runs this figure looks the same as that in Kruger (2023). In other model runs the y-axis scale is much larger (e.g. to 1e+05). In other words, the model is very unstable (simply re-run and plot the same model multiple times to confirm this fact). 
...
# Change prediction to 'confidence' (credible) interval
...
# Extreme / strange values for prediction intervals are perhaps expected given the large 
